...
/// Week-view calendar — meetings, calls, task deadlines, and reminders.
...
class _CalendarScreenState extends ConsumerState<CalendarScreen> {
  ...
  List<Map<String, dynamic>> _mergeEvents(
    ...
    final linkedTaskIds = events
        ...
        .whereType<String>()
        .toSet();
    final out = events.map((e) => Map<String, dynamic>.from(e)).toList();
    ...
    if (meId != null) {
      for (final task in tasks) {
        ...
        final mine = assignees.any((a) =>
            ((a['user'] as Map?)?['id'] ?? a['userId'])?.toString() == meId);
        if (!mine) continue;
        final due = DateTime.tryParse('${task['dueAt']}')?.toLocal();
        if (due == null || due.isBefore(_weekStart) || !due.isBefore(weekEnd)) {
          continue;
        }
        out.add({
          'id': 'task:$taskId',
          ...
          'subtitle': 'Task · due',
          'kind': 'task_deadline',
          'startsAt': due.toUtc().toIso8601String(),
          'taskId': taskId,
        });
      }
    }
    ...
    out.sort((a, b) {
      final as = DateTime.tryParse('${a['startsAt']}') ?? DateTime(2100);
      final bs = DateTime.tryParse('${b['startsAt']}') ?? DateTime(2100);
  ...
  void _openEvent(Map<String, dynamic> event) {
    ...
    final meetingSlug = event['meetingSlug']?.toString();
    if (taskId != null) {
      context.push('/tasks/$taskId');
      return;
    }
    if (callId != null) {
      context.push('/call/$callId?mode=voice');
      return;
    }
    if (meetingSlug != null) {
      context.push('/meeting/$meetingSlug?mode=video');
      return;
    }
          ...
          child: eventsAsync.when(
            ...
            data: (events) {
              ...
              final merged = _mergeEvents(
                events,
                tasks != null ? flattenTasksResponse(tasks) : const [],
                meId,
                weekEnd,
              );
              if (merged.isEmpty) {
                return const BestieEmptyState(
                  ...
                  description:
                      'No meetings, calls, or task deadlines this week.',
                );
              }
